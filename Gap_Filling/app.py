...
@st.cache_data 
def load_default_df():
    default_df_url = "https://raw.githubusercontent.com/saranobrega/Data/main/Arganildf.txt"
    return pd.read_csv(default_df_url)
...
def main():
    ...
    # Checkbox to choose between default and user-uploaded dataset
    use_default_dataset = st.checkbox("Use Default Dataset", value=False)

    # Load default DataFrame or user-uploaded DataFrame
    ...
    if df is not None:  # Check if df is defined
        ...
        for wt, df in data_without_outliers.items():
            ...
            gap_validation({wt: synthetic_data_dict})
